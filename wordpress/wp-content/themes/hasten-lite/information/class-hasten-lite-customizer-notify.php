<?php
/**
 * Hasten_Lite Customizer Notification System.
 *
 * @package Hasten_Lite
 */
 ...
 */
class Hasten_Lite_Customizer_Notify {
	...
	/**
	 * The single instance of Hasten_Lite_Customizer_Notify
	 *
	 * @var Hasten_Lite_Customizer_Notify $instance The Hasten_Lite_Customizer_Notify instance.
	 */
	...
	/**
	 * The Main Hasten_Lite_Customizer_Notify instance.
	 *
	 * We make sure that only one instance of Hasten_Lite_Customizer_Notify exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Hasten_Lite_Customizer_Notify ) ) {
			self::$instance = new Hasten_Lite_Customizer_Notify;
	...
	public function setup_actions() {
		...
		add_action( 'customize_register', array( $this, 'hasten_lite_customizer_notify_customize_register' ) );

		// Enqueue scripts and styles
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'hasten_lite_customizer_notify_scripts_for_customizer' ), 0 );

		/* ajax callback for dismissable recommended actions */
		add_action( 'wp_ajax_ti_customizer_notify_dismiss_recommended_action', array( $this, 'hasten_lite_customizer_notify_dismiss_recommended_action_callback' ) );

		add_action( 'wp_ajax_ti_customizer_notify_dismiss_recommended_plugins', array( $this, 'hasten_lite_customizer_notify_dismiss_recommended_plugins_callback' ) );

	}

	/**
	 * Scripts and styles used in the Hasten_Lite_Customizer_Notify class
	 */
	public function hasten_lite_customizer_notify_scripts_for_customizer() {
        ...
        wp_localize_script(
			'hl-customizer-notify-customizer-js', 'tiCustomizerNotifyObject', array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'hasten-lite' ),
			)
		);
	 ...
	 */
	public function hasten_lite_customizer_notify_customize_register( $wp_customize ) {

		/**
		 * Include the Hasten_Lite_Customizer_Notify_Section class.
		 */
		require_once get_template_directory() . '/information/class-hasten-lite-customizer-notify-section.php';

		$wp_customize->register_section_type( 'Hasten_Lite_Customizer_Notify_Section' );

		$wp_customize->add_section(
			new Hasten_Lite_Customizer_Notify_Section(
				$wp_customize,
				'hl-customizer-notify-section',
				array(
	 ...
	 */
	public function hasten_lite_customizer_notify_dismiss_recommended_action_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			if ( get_option( 'hasten_lite_customizer_notify_show_recommended_actions' ) ) {

				$ti_customizer_notify_show_recommended_actions = get_option( 'hasten_lite_customizer_notify_show_recommended_actions' );
				switch ( $_GET['todo'] ) {
					case 'add':
				...
				update_option( 'hasten_lite_customizer_notify_show_recommended_actions', $ti_customizer_notify_show_recommended_actions );
				...
				if ( ! empty( $ti_customizer_notify_recommended_actions ) ) {
					...
					update_option( 'hasten_lite_customizer_notify_show_recommended_actions', $ti_customizer_notify_show_recommended_actions_new );
				}
			}
		}
	 ...
	 */
	public function hasten_lite_customizer_notify_dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			/* if the option exists, update the record for the specified id */
			$ti_customizer_notify_show_recommended_plugins = get_option( 'hasten_lite_customizer_notify_show_recommended_plugins' );
			...
			update_option( 'hasten_lite_customizer_notify_show_recommended_plugins', $ti_customizer_notify_show_recommended_plugins );
		}
		die(); // this is required to return a proper result
	}
