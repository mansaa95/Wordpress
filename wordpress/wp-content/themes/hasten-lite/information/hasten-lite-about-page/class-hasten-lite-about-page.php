 ...
 *                      'button_link' => esc_url( 'https://hasten-lite.com/contact/' ),
 *                      'is_button' => true,
 *                      'is_new_tab' => false
 *                  ),
 *            ),
 ...
 *                'pro_theme_link'      => 'https://hasten-lite.com/themes/flymag-pro/',
 ...
 *                    ),
 *                    array(
 *                        'link'        => 'http://hasten-lite.com/plugins/easy-content-types/',
 *                    ),
 *                    array(
 ...
 *            ),
 *        );
 *        Hasten_Lite_About_Page::init( $config );
 *
 * @package Hasten_Lite
 * @subpackage Admin
 * @since 1.0.0
 */
if ( ! class_exists( 'Hasten_Lite_About_Page' ) ) {
	 ...
	 */
	class Hasten_Lite_About_Page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The Hasten_Lite_About_Page class version.
		 */
		...
		/**
		 * The single instance of Hasten_Lite_About_Page
		 *
		 * @var Hasten_Lite_About_Page $instance The  Hasten_Lite_About_Page instance.
		 */
		private static $instance;

		/**
		 * The Main Hasten_Lite_About_Page instance.
		 *
		 * We make sure that only one instance of Hasten_Lite_About_Page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Hasten_Lite_About_Page ) ) {
				self::$instance = new Hasten_Lite_About_Page;
		...
		public function activation_admin_notice() {
			...
			if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
				add_action( 'admin_notices', array( $this, 'hasten_lite_about_page_welcome_admin_notice' ), 99 );
			}
		}
		 ...
		 */
		public function hasten_lite_about_page_welcome_admin_notice() {
		 ...
		 */
		public function hasten_lite_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap hasten-about-wrap about-wrap epsilon-wrap">';
...
//
        ...
        public function demo_import()
        {
            if (in_array('one-click-demo-import/one-click-demo-import.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                    ...
                    if (!empty($support_steps)) {
                        ...
                        foreach ($support_steps as $support_step) {
                            ...
                            if ($support_step['is_free_pro'] == 'free')
                                echo '<div class="hasten-lite-ribbon hasten-lite-free"></div>';
                            else
                                echo '<div class="hasten-lite-ribbon hasten-lite-pro"></div>';
                ...
                echo '<div class="theme-browser rendered">';
                echo '<div class="themes wp-clearfix">';
                echo sprintf( wp_kses( __( 'Please Install One Click Demo Import Plugin For Demos. <a href="%s">Download Here.</a>', 'hasten-lite' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( 'https://wordpress.org/plugins/one-click-demo-import/' ) );
                echo '</div>';
                echo '</div>';
            }
        }
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<div id="free_pro" class="hl-about-page-tab-pane hl-about-page-fre-pro">';
					echo '<table class="free-pro-table">';
					echo '<thead>';
					echo '<tr class="hl-about-page-text-right">';
					echo '<th></th>';
					...
					foreach ( $free_pro['features'] as $feature ) {
                        ...
                        elseif(($feature['is_in_lite'] != 'true' ) && ( $feature['is_in_lite'] != 'false' ) ){
                            echo '<td class="only-pro">'.esc_html($feature['is_in_lite']).'</td>';
                        }
						else {
                        ...
                        elseif(($feature['is_in_pro'] != 'true' ) && ( $feature['is_in_pro'] != 'false' ) ){
                            echo '<td class="only-pro">'.esc_html($feature['is_in_pro']).'</td>';
                        }
						else {
							echo '<td class="only-pro"><span class="dashicons-before dashicons-no-alt"></span></td>';
						}
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						...
						echo '</td>';
						echo '<td colspan="2" class="hl-about-page-text-right"><a href="' . esc_url( $free_pro['pro_theme_link'] ) . '" target="_blank" class="button button-primary button-hero">' . wp_kses_post( $free_pro['get_pro_theme_label'] ) . '</a></td>';
						echo '</tr>';
					}
					echo '</tbody>';
					echo '</table>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'hl-about-page-css', get_template_directory_uri() . '/information/hasten-lite-about-page/css/hasten_lite_about_page_css.css', array(), HASTEN_LITE_VERSION );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'hl-about-page-js', get_template_directory_uri() . '/information/hasten-lite-about-page/js/hasten_lite_about_page_scripts.js', array( 'jquery' ), HASTEN_LITE_VERSION );
				...
				wp_localize_script(
					'hl-about-page-js', 'tiAboutPageObject', array(
						...
						'activating_string'   => esc_html__( 'Activating', 'hasten-lite' ),
					)
				);
